...
app.patch('/posts/:id', (req, res) => {
  const postId = parseInt(req.params.id)
  const existingPosts = posts.find((post) => post.id === postId)
  const updatedPosts = {
    id: postId,
...
app.delete('/posts/:id', (req, res) => {
  ...
  if (postIndex > -1) {
    posts.splice(postIndex, 1) // An array containing the elements that were deleted.
    res.status(200);
  } else {
    res.status(403)
    res.json({
      error: `Post with id: ${postId} not found. No Post has been deleted...`
    })
  }
